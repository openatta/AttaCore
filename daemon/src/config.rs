...
impl DefaultDaemonPaths {
    pub fn from_env(scope: &str) -> Self {
        ...
        Self { global_root, config_root, project_root }
    }
    ...
    pub fn new(config_root: PathBuf, project_root: PathBuf) -> Self {
        ...
        Self { global_root: config_root.clone(), config_root, project_root }
    }
}
...
impl DaemonPaths for DefaultDaemonPaths {
    ...
    fn global_root(&self) -> PathBuf { self.global_root.clone() }
}
...
impl StaticDaemonPaths {
    pub fn new(path: PathBuf) -> Self {
        Self { config_root: path.clone(), project_root: path, global_root: None }
    }
    pub fn with_project(config_root: PathBuf, project_root: PathBuf) -> Self {
        Self { config_root, project_root, global_root: None }
    }
...
impl DaemonPaths for StaticDaemonPaths {
    ...
    fn project_root(&self) -> PathBuf { self.project_root.clone() }
    fn global_root(&self) -> PathBuf {
        self.global_root.clone().unwrap_or_else(|| self.config_root.clone())
    }
}
...
impl std::fmt::Debug for DaemonConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DaemonConfig")
            ...
            .field("mcp_servers", &self.settings.mcp_servers.keys().collect::<Vec<_>>())
            .field("tcp_addr", &self.tcp_addr)
            .field("tcp_token", &"...")
            .field("paths", &"...")
            .field("permission_rules", &format!("RuleSet({})", self.permission_rules.len()))
            .field("providers", &self.settings.providers.keys().collect::<Vec<_>>())
            .field("default_provider", &self.settings.default_provider)
            .field("task_models", &self.settings.task_models.keys().collect::<Vec<_>>())
...
mod tests {
    ...
    fn default_daemon_paths_config_root_nests_under_global_root_scenes() {
        ...
        assert_eq!(paths.config_root(), paths.global_root().join("scenes").join("coding"));
    ...
    fn load_daemon_config_project_overrides_user() {
        ...
        write_settings(config_dir.path(), r#"{"model": {"model_name": "user-model"}, "execution": {"max_parallelism": 1, "max_api_calls_per_turn": 1}}"#);
        write_project_settings(project_dir.path(), r#"{"model": {"model_name": "project-model"}}"#);
    ...
    fn load_daemon_config_global_is_lowest_priority() {
        ...
        let config_dir = tempfile::tempdir().unwrap();
        let project_dir = tempfile::tempdir().unwrap();
        write_settings(global_dir.path(), r#"{"model": {"model_name": "global-model"}}"#);
        write_settings(config_dir.path(), r#"{"model": {"model_name": "scene-model"}}"#);
    ...
    fn load_daemon_config_parses_and_merges_providers() {
        ...
        assert_eq!(config.settings.default_provider.as_deref(), Some("deepseek"));
        assert_eq!(
    ...
    fn load_daemon_config_parses_and_merges_hooks() {
        ...
        let hooks = config.settings.hooks_config.expect("hooks should be present");
