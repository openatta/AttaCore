...
///
/// Only `api_type: anthropic` (or unset — same default as
/// `base::provider::ApiType`'s serde attribute) has a runtime
/// implementation today. `openai_compatible` is a valid *config* value
/// (§3.1 of the design doc) but there's no `Model` impl for it yet — that's
/// a hard error surfaced at startup (via `Err`, which `main.rs` turns into
/// `anyhow::bail!`), not a silent downgrade to Anthropic or a runtime panic
/// on first use.
fn build_provider_model(
    ...
    match cfg.api_type.as_deref() {
        Some("openai_compatible") => Err(format!(
            "provider '{provider_id}': api_type 'openai_compatible' has no runtime \
             implementation yet (only 'anthropic' is wired) — see \
             docs/design/2026-08-04-multi-provider-llm-migration.md §6 phase 2"
        )),
...
mod tests {
    ...
    fn build_task_router_propagates_a_single_bad_providers_error() {
        ...
        providers.insert("broken".to_string(), {
            let mut p = anthropic_provider("key2");
            p.api_type = Some("openai_compatible".into());
            p
        });
        let err = err_of(build_task_router(&providers, "anthropic", HashMap::new()));
