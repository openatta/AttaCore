...
fn inspect_tier(tier: &'static str, dir: &Path) -> TierStatus {
    ...
    if !path.exists() {
        return TierStatus { tier, path: path_str, exists: false, parses: true, error: None };
    }
    match std::fs::read_to_string(&path) {
        Ok(text) => match serde_json::from_str::<serde_json::Value>(&text) {
            Ok(_) => TierStatus { tier, path: path_str, exists: true, parses: true, error: None },
            Err(e) => TierStatus {
                tier,
                path: path_str,
                exists: true,
...
mod tests {
    ...
    fn reports_missing_tiers_as_not_existing_but_ok() {
        ...
        let report = run_doctor(&paths, "coding", &settings, false);
    ...
    fn reports_malformed_tier_as_not_parsing() {
        ...
        assert!(tiers.iter().any(|t| t["exists"] == true && t["parses"] == false));
    }
    ...
    fn reports_history_store_wiring_status() {
        ...
        let paths = StaticDaemonPaths::new(dir.path().to_path_buf());
        let settings = base_settings();
        let report = run_doctor(&paths, "coding", &settings, true);
