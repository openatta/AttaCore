...
| `files` | 否 | 随 skill 一起打包的参考文件列表，路径相对于 skill 所在目录（比如 `files: [notes.md, ref/extra.md]`）。skill 被调用时，这些文件的内容会被读出并追加注入到发给模型的内容里，每个文件单独包一段 `## Bundled file: <相对路径>` 标题。有独立的字符预算：单文件最多 2000 字符，所有文件合计最多 6000 字符（都是在正文本身 8000 字符预算之外另算，不会互相挤占）；超预算的部分截断并标注 `...[truncated]`，总预算超限后剩余文件直接跳过并注明原因。路径必须落在 skill 自己的目录内——用 `..` 之类试图跳出该目录的声明会被拒绝，调用直接返回 `is_error: true` 并点名是哪个 skill、哪个路径，不会静默跳过或截断了事；文件不存在/读不出来也是同样处理 |
