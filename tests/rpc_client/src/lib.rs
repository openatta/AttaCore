...
impl DaemonRpcClient {
    pub async fn connect(socket_path: &Path) -> anyhow::Result<Self> {
        ...
        Ok(Self { reader: BufReader::new(read_half), writer, next_id: AtomicI64::new(1) })
    }
    ...
    pub async fn session_close(&mut self, session_id: &str) -> anyhow::Result<RpcResponse> {
        self.call("session.close", serde_json::json!({ "session_id": session_id })).await
    }
    ...
    pub async fn session_run_turn(
        ...
        loop {
            line.clear();
            let n = self.reader.read_line(&mut line).await?;
            anyhow::ensure!(n > 0, "daemon connection closed mid-turn (session={session_id})");
            let trimmed = line.trim();
            if trimmed.is_empty() {
                continue;
            }
    ...
    pub async fn config_get_provider(&mut self, include_secrets: bool) -> anyhow::Result<RpcResponse> {
        self.call("config.getProvider", serde_json::json!({ "include_secrets": include_secrets })).await
    }
    ...
    pub async fn mcp_add_server(&mut self, name: &str, config: Value) -> anyhow::Result<RpcResponse> {
        self.call("mcp.addServer", serde_json::json!({ "name": name, "config": config })).await
    }
    ...
    pub async fn import_run(&mut self, source: &str) -> anyhow::Result<RpcResponse> {
        self.call("import.run", serde_json::json!({ "source": source })).await
    }
    ...
    pub async fn plugin_set_enabled(&mut self, name: &str, enabled: bool) -> anyhow::Result<RpcResponse> {
        let method = if enabled { "plugin.enable" } else { "plugin.disable" };
