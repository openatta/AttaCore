...
async fn plugin_install_list_uninstall_round_trips() {
    ...
    let (zip_path, checksum) =
        test_runner::plugin_fixture::package_demo_plugin(&tmp.join("packaged")).expect("package demo-plugin");
    ...
    let mut client = DaemonRpcClient::connect(&socket_path).await.expect("connect");
    ...
    assert!(install_resp.error.is_none(), "plugin.install failed: {:?}", install_resp.error);
    ...
    let names: Vec<&str> = plugins["plugins"]
        ...
        .collect();
    assert!(names.contains(&"demo-plugin"), "expected demo-plugin in plugin.list, got: {names:?}");

    let uninstall_resp = client
        .call("plugin.uninstall", serde_json::json!({ "name": "demo-plugin" }))
        .await
        .expect("plugin.uninstall RPC");
    assert!(uninstall_resp.error.is_none(), "plugin.uninstall failed: {:?}", uninstall_resp.error);
