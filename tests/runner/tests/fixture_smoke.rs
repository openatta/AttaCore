...
fn fixture_dir() -> &'static Path {
    Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/../fixtures/template_project"))
}
...
fn settings_json_hooks_config_parses() {
    ...
    assert!(hooks.contains_key(&hooks::HookEvent::PreToolUse), "expected a PreToolUse hook");
}
...
fn custom_agent_type_frontmatter_parses() {
    ...
    assert!(!types.is_empty(), "expected at least one *.md agent type under .atta/agents/");
    assert!(types.iter().any(|t| t.name == "reviewer"), "expected the 'reviewer' agent type to parse");
}
...
fn settings_load_resolves_project_root_and_config_from_fixture() {
    ...
    assert_eq!(
        settings.paths.project_root(), fixture_dir().to_path_buf(),
        "project_root() should resolve to the fixture root"
    );
    let hooks = settings.hooks_config.expect("hooks_config should load from fixture's .atta/settings.json");
    assert!(hooks.get("PreToolUse").is_some());
    assert!(!settings.mcp_servers.is_empty(), "mcp_servers should load from fixture's .atta/settings.json");
...
fn mcp_toy_server_placeholder_resolves_to_real_binary() {
    ...
    let content = std::fs::read_to_string(tmp.join(".atta/settings.json")).expect("read copied settings.json");
    assert!(!content.contains("{{MCP_TOY_SERVER_BIN}}"), "placeholder should be fully substituted");

    let v: serde_json::Value = serde_json::from_str(&content).unwrap();
    let command = v["mcp_servers"]["demo"]["command"].as_str().expect("command is a string");
    let path = Path::new(command);
    assert!(path.is_absolute(), "resolved command should be an absolute path, got: {command}");
    let meta = std::fs::metadata(path).unwrap_or_else(|e| panic!("resolved mcp-toy-server binary should exist at {command}: {e}"));
    use std::os::unix::fs::PermissionsExt;
    assert!(meta.permissions().mode() & 0o111 != 0, "resolved binary should be executable");
...
fn hook_script_is_executable() {
    ...
    assert!(meta.permissions().mode() & 0o111 != 0, "pre_bash_log.sh must be executable");
}
