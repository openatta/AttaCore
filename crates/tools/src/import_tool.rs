...
mod tests {
    ...
    async fn lists_nothing_for_empty_project() {
        ...
        let tool = ImportTool;
        let r = tool
            .call(json!({}), ctx_at(dir.path()), ProgressSender::noop("t"))
            .await
            .unwrap();
        assert!(!r.is_error);
        match r.content {
    ...
    async fn manual_command_ignores_prior_skipped_marker() {
        ...
        tokio::fs::write(dir.path().join("CLAUDE.md"), "be concise")
            .await
            .unwrap();
        base::frozen::mark_skipped(dir.path(), &[], None)
            .await
            .unwrap();
        ...
        match r.content {
            base::tool::ToolResultContent::Text(ref t) => {
                assert!(
                    t.contains("claude_code"),
                    "must still list sources despite skipped marker"
                );
            }
            _ => panic!("expected text"),
        }
