...
mod tests {
    ...
    impl MockCallback {
        fn new(decision: ImportDecision) -> Self {
            Self { decision, delay: Duration::ZERO, calls: AtomicUsize::new(0) }
        }

        fn with_delay(decision: ImportDecision, delay: Duration) -> Self {
            Self { decision, delay, calls: AtomicUsize::new(0) }
        }
    ...
    async fn no_callback_registered_does_nothing() {
        ...
        assert!(!import_already_decided(dir.path()).await, "no marker should be written");
    }
    ...
    async fn no_sources_never_invokes_callback() {
        ...
        assert!(result.is_none());
        assert_eq!(mock.call_count(), 0, "no importable sources means the callback is never invoked");
    }
    ...
    async fn already_decided_project_never_invokes_callback() {
        ...
        let result = maybe_detect_and_import(dir.path(), Some(&cb), Duration::from_secs(5)).await;
        assert!(result.is_none());
        assert_eq!(mock.call_count(), 0, "already-decided project must not invoke the callback");
    }
    ...
    async fn import_decision_executes_and_marks_imported() {
        ...
        let cb: Arc<dyn ImportCallback> =
            Arc::new(MockCallback::new(ImportDecision::Import(crate::frozen::ImportSourceKind::ClaudeCode)));
        let result = maybe_detect_and_import(dir.path(), Some(&cb), Duration::from_secs(5)).await;
    ...
    async fn skip_decision_marks_skipped_without_executing() {
        ...
        assert!(result.is_none());
        assert!(
            tokio::fs::metadata(dir.path().join("AGENTS.md")).await.is_err(),
            "skip must not write AGENTS.md"
        );
        assert!(import_already_decided(dir.path()).await, "skip must persist a marker");
    }
    ...
    async fn defer_decision_leaves_no_marker() {
        ...
        let result = maybe_detect_and_import(dir.path(), Some(&cb), Duration::from_secs(5)).await;
        assert!(result.is_none());
        assert!(!import_already_decided(dir.path()).await, "defer must not persist a marker — ask again next time");
    }
