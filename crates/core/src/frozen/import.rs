...
impl ImportSource {
    ...
    pub fn describe(&self) -> String {
        match self {
            ImportSource::ClaudeCode { claude_md, skills_dir } => {
            ...
            ImportSource::Codex => {
                "Codex (AGENTS.md present, .agents/skills/ missing)".to_string()
            }
            ImportSource::Cursor { legacy_rules, mdc_files } => {
                let mut parts = Vec::new();
...
pub async fn detect_import_sources(cwd: &Path) -> Vec<ImportSource> {
    ...
    let legacy_rules = path_exists(&cursorrules_path).await.then_some(cursorrules_path);
    let mdc_files = scan_cursor_mdc_files(&cwd.join(".cursor").join("rules")).await;
    if legacy_rules.is_some() || !mdc_files.is_empty() {
        sources.push(ImportSource::Cursor { legacy_rules, mdc_files });
    }
...
pub async fn already_decided(cwd: &Path) -> bool {
    ...
    matches!(marker.decision.as_deref(), Some("imported") | Some("skipped"))
}
...
pub async fn mark_imported(
    ...
    let marker = ImportedMarker {
        ...
        sources_seen: sources_seen.iter().map(|s| s.kind().as_str().to_string()).collect(),
        skipped_reason: None,
    };
...
pub async fn execute_import(cwd: &Path, source: &ImportSource) -> Result<ImportSummary, ImportError> {
        ...
        ImportSource::ClaudeCode { claude_md, skills_dir } => {
            if let Some(claude_md_path) = claude_md {
                if let Ok(content) = tokio::fs::read_to_string(claude_md_path).await {
                    ...
                    actions.push(format!("merged {} into AGENTS.md", claude_md_path.display()));
                }
            }
    ...
    Ok(ImportSummary { kind: source.kind(), actions })
}

async fn merge_into_agents_md(cwd: &Path, content: &str, marker_tag: &str) -> Result<(), ImportError> {
    let agents_path = cwd.join("AGENTS.md");
    let existing = tokio::fs::read_to_string(&agents_path).await.unwrap_or_default();
...
async fn append_rule_reference(cwd: &Path, rule_path: &str) -> Result<(), ImportError> {
    let agents_path = cwd.join("AGENTS.md");
    let mut existing = tokio::fs::read_to_string(&agents_path).await.unwrap_or_default();
...
mod tests {
    ...
    async fn detects_claude_md() {
        let dir = TempDir::new().unwrap();
        tokio::fs::write(dir.path().join("CLAUDE.md"), "# instructions").await.unwrap();
        let sources = detect_import_sources(dir.path()).await;
    ...
    async fn detects_codex_gap() {
        let dir = TempDir::new().unwrap();
        tokio::fs::write(dir.path().join("AGENTS.md"), "# instructions").await.unwrap();
        let sources = detect_import_sources(dir.path()).await;
        assert_eq!(sources.len(), 1);
    ...
    async fn no_codex_gap_when_agents_skills_present() {
        let dir = TempDir::new().unwrap();
        tokio::fs::write(dir.path().join("AGENTS.md"), "# instructions").await.unwrap();
        tokio::fs::create_dir_all(dir.path().join(".agents/skills")).await.unwrap();
        let sources = detect_import_sources(dir.path()).await;
        assert!(sources.is_empty());
    }
    ...
    async fn detects_cursor_legacy_rules_and_mdc() {
        let dir = TempDir::new().unwrap();
        tokio::fs::write(dir.path().join(".cursorrules"), "be nice").await.unwrap();
        let rules_dir = dir.path().join(".cursor/rules");
        tokio::fs::create_dir_all(&rules_dir).await.unwrap();
        tokio::fs::write(rules_dir.join("style.mdc"), "---\ndescription: style\n---\nbody")
            .await
            .unwrap();
        let sources = detect_import_sources(dir.path()).await;
        assert_eq!(sources.len(), 1);
    ...
    async fn detects_multiple_sources_simultaneously() {
        let dir = TempDir::new().unwrap();
        tokio::fs::write(dir.path().join("CLAUDE.md"), "x").await.unwrap();
        tokio::fs::write(dir.path().join(".cursorrules"), "y").await.unwrap();
        let sources = detect_import_sources(dir.path()).await;
    ...
    async fn already_decided_true_after_mark_imported() {
        let dir = TempDir::new().unwrap();
        mark_imported(dir.path(), &[], ImportSourceKind::ClaudeCode).await.unwrap();
        assert!(already_decided(dir.path()).await);
    }
    ...
    async fn already_decided_true_after_mark_skipped() {
        let dir = TempDir::new().unwrap();
        mark_skipped(dir.path(), &[], Some("user said no".into())).await.unwrap();
        assert!(already_decided(dir.path()).await);
    }
    ...
    async fn execute_claude_code_merges_agents_md_and_copies_skills() {
        ...
        tokio::fs::write(skill_src.join("SKILL.md"), "---\ndescription: demo\n---\nbody")
            .await
            .unwrap();
    ...
    async fn execute_claude_code_does_not_clobber_existing_project_skill() {
        ...
        tokio::fs::write(existing_dest.join("SKILL.md"), "---\ndescription: NEW\n---\nbody")
            .await
            .unwrap();
        ...
        let content = tokio::fs::read_to_string(existing_dest.join("SKILL.md")).await.unwrap();
        assert!(content.contains("NEW"), "existing project skill must not be overwritten");
    }

    #[tokio::test]
    async fn execute_codex_creates_empty_skills_dir() {
        let dir = TempDir::new().unwrap();
        let summary = execute_import(dir.path(), &ImportSource::Codex).await.unwrap();
        assert_eq!(summary.kind, ImportSourceKind::Codex);
        assert!(tokio::fs::metadata(dir.path().join(".agents/skills")).await.is_ok());
    }
    ...
    async fn execute_cursor_merges_rules_and_converts_mdc() {
        ...
        tokio::fs::write(&cursorrules, "always use tabs").await.unwrap();
        let mdc_path = dir.path().join(".cursor/rules/style.mdc");
        tokio::fs::create_dir_all(mdc_path.parent().unwrap()).await.unwrap();
        tokio::fs::write(
            &mdc_path,
        ...
        assert!(agents_md.contains(".atta/rules/style.md"), "always-apply rule must be referenced");

        let rule_content = tokio::fs::read_to_string(dir.path().join(".atta/rules/style.md")).await.unwrap();
