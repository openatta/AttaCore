...
//!
//! Instead of waiting until the token budget is critically low, reactive
//! compaction predicts when compaction will be needed and fires early
//! (typically a MicroCompact) to avoid stalling the user with a last-minute
//! big compaction.
//!
...
/// Escalation order: Normal → Warning → Auto → Blocking.
/// Warning and Error use the same buffer offset, so they map to the same level.
